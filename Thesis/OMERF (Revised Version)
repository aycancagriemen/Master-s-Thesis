#The following R scripts were taken from G Bergonzoli’s (2024) work and were slightly revised to be applied to the random intercept model. 
#The offset variable in the clmm model formulation, which is used to estimate the random effect part in the algorithm, seems not to be working. 
#Therefore, to get the benefit of offset variable in the formula, the formula was revisited by utilizing the “stats” package in R.

#Definition of parameters used in the function
#y = response variable in the dataset
#cov = all covariates except response, time and grouping variable
#group = grouping variable
#xnam = all covariates (fixed variable) to be used in the random forest algorithm
#znam = NULL
#bizero = NULL
#itmax = 100
#toll = 0.05

OMERF_new= function (y, cov, group, xnam=NULL, znam=NULL, bizero=NULL, 
                 itmax=100, toll=0.05) {
  ...
  z.not.null=!(is.null(znam))
  if(z.not.null) {
    Zi = cov[NULL]
  } else {
    Zi = cov[znam]
  }
  #check whether there are covariates included in the random effects
  ...
  if(z.not.null) {
    clmm.formula=as.formula(paste("y ~ stats::offset(eta.est.rf) + ( 1+", paste(znam, collapse= "+"), " | group )"))
  } else {
    clmm.formula=as.formula(paste("y ~ stats::offset(eta.est.rf) + ( 1 | group)"))
  }
  ...
  while(!converged && it<itmax) {
    ...
    clmm.fit= clmm(clmm.formula,link='logit', data=clmm.data,nAGQ = 4 ,Hess=TRUE, control=clmm.control(maxLineIter = 500, maxIter=1000, grtol=1e-3))
